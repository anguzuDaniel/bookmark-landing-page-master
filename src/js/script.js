...
curoselBtn.forEach((btn) => {
	...
	btn.addEventListener("click", (e) => {
		...
		if (!btn.classList.contains("features__btn--active")) {
			...
			for (let i = 0; i < curoselBtn.length; i++) {
				// if the button at position curosel[i] button is not the button clicked the active class is removed
				...
				if (curoselBtn[i] !== btn) {
					...
					// checks if current description doesn't have active class and adds if has
...
dropdownBtn.forEach((btn) => {
	btn.addEventListener("click", (e) => {
		...
		if (btn.parentNode.querySelector(".dropdown__paragragh")) {
			btn.parentNode.querySelector(".dropdown__paragragh").classList.toggle("closed");
		}
	});
});
